...
1. `plugin/` 에서 빌드 → `plugin/target/rpgcore-plugin-2.4.1.jar`
   ```
...
선포하면 공짜"가 되지 않습니다. 그리고 **자기 깃발을 스스로 부수면 항복 처리**됩니다 —
안 그러면 질 것 같을 때 목표물을 없애 무승부로 만드는 게 최선의 수가 됩니다.
...
`plugins/RPGCorePlugin/mailbox.yml` 에 저장됩니다. 둘 다 **바뀔 때마다 즉시** 기록합니다 —
이 파일들이 존재하는 이유가 바로 "서버가 곱게 안 내려간 경우"이기 때문입니다.
