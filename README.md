...
업적과 칭호, 수집 도감, 1:1 대결, **골드 경매장**, **길드와 영지**, 소지 무게,
...
1. `plugin/` 에서 빌드 → `plugin/target/rpgcore-plugin-2.3.0.jar`
   ```
...
길드는 `plugins/RPGCorePlugin/guilds.yml` 에 저장됩니다. 명단과 영지뿐 아니라 **공동 보관함
내용물도 같은 파일에** 들어갑니다 — 누가 열 수 있는지를 적은 명단과 그 안의 물건이 서로
...
plugin/
  ...
  src/main/java/com/rpgcore/plugin/
    ...
    menu/                    /menu 허브 화면
    guild/                   길드, 공동 보관함, 영지(청크 색인·보호·버프·성장)
