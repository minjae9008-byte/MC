...
업적과 칭호, 수집 도감, 1:1 대결, **골드 경매장**, 소지 무게, 장비 내구도 페널티,
연쇄 벌목, 모루 커스텀 강화, 근접 채팅을 한 덩어리로 제공합니다.
...
1. `plugin/` 에서 빌드 → `plugin/target/rpgcore-plugin-2.2.0.jar`
   ```
...
### `/menu` — 전부 여기서
상자 창 하나에서 스탯 · 직업 · 칭호 · 도감 · 업적 · 순위표 · 파티 · 경매장 · 우편함으로
바로 갑니다. 화면끼리 서로 오갈 수 있으므로 명령어를 몰라도 기능을 전부 찾을 수 있습니다.
...
사람이 직접 해 봐야 하는 것: `/auction sell` 과 GUI 에서의 입찰·즉시구매·내리기.
